 ...
 * Author: Howard Chen
 * Date: 8-9-2017
 * Description: A program that connects to otp_enc_d and asks it to send back an encrypted version
 * 		of a message.
 *
 * 		Usage: otp_enc <plaintext> <key> <port>, where plaintext is the file that contains
 * 		the plaintext to be encrypted, key is the encryption key that will be used to encrypt
 * 		the text, and port is the port that otp_enc should try to connect to otp_enc_d on.
 *
 */
 ...
 * 		end of the file */
	...
	plaintext_name = argv[1];
	plaintext = readFile(plaintext_name, &plaintext_length);
	key_name = argv[2];
